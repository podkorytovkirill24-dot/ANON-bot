...
ADMIN_CHAT_IDS = (1218622472, 7576665621)
...
def load_config() -> Config:
    if not BOT_TOKEN.strip():
        raise RuntimeError("BOT_TOKEN is empty in main.py")
    if not ADMIN_CHAT_IDS:
        raise RuntimeError("ADMIN_CHAT_IDS is empty in main.py")
    if not CHANNEL_ID.strip():
        raise RuntimeError("CHANNEL_ID is empty in main.py")

    return Config(
        bot_token=BOT_TOKEN.strip(),
        admin_chat_ids=tuple(int(admin_id) for admin_id in ADMIN_CHAT_IDS),
        channel_id=CHANNEL_ID.strip(),
        channel_name=CHANNEL_NAME.strip() or "Лицей",
        db_path=DB_PATH.strip() or "queue.db",
    )
